...
.\.venv\Scripts\python -m pytest tests/ -v       # ~559 tests, verificado 2026-09-04 (0 fallidos). 1 skipped (test no-Windows, esperado en este SO).
...
- **`fs_find_duplicates` (Layer 1, v1.4.42, mejorada en v1.4.80)**: búsqueda de duplicados exactos por contenido (SHA256) dentro de un `path`, a diferencia de buscar por patrón de nombre. Diseño de dos fases sin límite de cantidad ni tamaño de archivo por defecto (decisión explícita, 2026-07-31): fase 1 agrupa por tamaño exacto en bytes (`stat()`, prácticamente gratis — medido en 240ms para 232 archivos reales); fase 2 solo calcula hash dentro de los grupos que ya comparten tamaño con al menos otro archivo. Un archivo de tamaño único, por grande que sea, nunca se hashea — así se evita tanto el coste de hashear innecesariamente como el error de excluir archivos grandes que es justamente lo que se busca auditar. Parámetro `extensions` acepta `".pdf"` o `"pdf"` indistintamente (normalización case-insensitive). **v1.4.80**: (1) nuevo parámetro `exclude` con poda real del recorrido (migración de `rglob`/`iterdir` a `os.walk` + `dirnames[:]`, reutilizando los helpers de v1.4.79) — matchear `node_modules`/`.venv` sin podar no ahorraría el hashing de miles de dependencias repetidas; (2) filtros de tamaño opt-in `min_size` (default 0) / `max_size` (default None) aplicados en fase 1 antes de agrupar — el default conserva el diseño "sin caps"; (3) los archivos vacíos (size 0) se ignoran SIEMPRE (consenso jdupes/rmlint verificado contra manpages, 2026-08-16: `-n --noempty` default en jdupes, `--size` default 1 en rmlint; czkawka con 1 KB es la excepción — cubierto con `min_size=1024` explícito). ORIGINAL = el más viejo por `st_ctime`; grupos ordenados por espacio desperdiciado (`size × (copias−1)`). Solo lectura, no borra nada — deliberadamente separada de `fs_delete_batch`.
- **`project_git_status` (Layer 4, v1.4.43, `path` opcional desde v1.4.48)**: estado de git para los repos encontrados bajo `paths_allow`, o bajo un `path` puntual si se pasa uno (validado con `security.resolve_and_validate()`, mismo mecanismo que `project_scan`/`project_find`). Descubrimiento vía `os.walk()` con poda de directorios (`node_modules`, `.venv`, `AppData`, etc.) — no `Path.rglob`, que no permite saltar subárboles completos una vez que entra. Sin límite de cantidad de repos, mismo principio que `fs_find_duplicates`. Reutiliza `_git_project_info()` (ya usada por `project_scan`), extendida con `ahead`/`behind` contra el upstream — `None` en esos campos significa "sin upstream configurado", distinto de `0` ("sincronizado"). **Guard v1.4.48**: si se omite `path` y alguna raíz de `paths_allow` es una raíz de disco completa (`_is_filesystem_root()`), no escanea — devuelve un mensaje pidiendo un `path` puntual, en vez de colgarse recorriendo todo el disco (incidente real: 4+ min de timeout con `paths_allow=["C:\\"]`, ver CHANGELOG 1.4.48). Las llamadas git por repo siguen siendo secuenciales, no paralelizadas — deuda conocida desde v1.4.11, sin resolver todavía. Esta es la primera de cuatro tools nuevas agregadas en la misma tanda (las tres siguientes abajo). **Uso (regla práctica): pasar SIEMPRE `path` puntual validado contra `paths_allow` (ej. `project_git_status(path="C:\\Users\\usuario\\Repos")`). Si se omite `path` y `paths_allow` incluye una raíz de disco completa (config actual del deployment: `["C:\\"]`), la tool devuelve el mensaje del guard pidiendo un `path` puntual — es el comportamiento esperado del guard v1.4.48, no un fallo; reintentar con `path` explícito, nunca reintentar sin él.**
- **`fs_disk_usage` (Layer 1, v1.4.44, mejorada en v1.4.79 y v1.4.81)**: agrupa el tamaño de todos los archivos bajo un `path` por carpeta ancestro a `depth` niveles, devuelve las `top_n` que más pesan. Complementa a `fs_find_duplicates` (esa responde "qué está repetido", esta responde "qué carpeta pesa más"). Un solo `os.walk()` sobre todo el árbol, atribuyendo cada archivo a su ancestro correspondiente en un solo pase — evita recorrer subárboles compartidos una vez por carpeta hermana. Sin límite de cantidad de carpetas ni de archivos escaneados, mismo principio que las dos anteriores; solo la salida (`top_n`) se trunca. **v1.4.79**: (1) cada bucket reporta además el conteo de archivos (el walk ya itera cada filename — gratis); (2) nuevo parámetro `exclude` con patrones fnmatch estilo `paths_deny` (`"**/node_modules/**"`, `"node_modules"`, `".venv"`), que PODA los directorios matcheados del `os.walk` antes de descender (excluir `node_modules` sin podar no ahorraría tiempo) y omite los archivos matcheados. Ojo con la semántica de `depth>1`: los ancestros intermedios NO aparecen como bucket propio — con `depth=2` el archivo en `a/b/c/x` se atribuye a `a/b`, no a `a` (los archivos directos de `a` caen a `path` mismo). **v1.4.81**: (1) filtros de tamaño opt-in `min_size` (default 0) / `max_size` (default None) con la MISMA semántica que `fs_find_duplicates` — un archivo fuera del rango no cuenta en su bucket ni en el total (el default conserva el comportamiento histórico); a diferencia de duplicados, NO ahorran I/O (el walk stastea todo igual) — solo cambian el significado del reporte; (2) los archivos vacíos (size 0) se ignoran SIEMPRE, mismo criterio que `fs_find_duplicates` — el conteo significa "archivos que ocupan espacio" (investigación rmlint: los vacíos son categoría de lint aparte `emptyfiles`/`emptydirs`, nunca mezclada con auditoría de espacio; inventariar carpetas vacías sería una tool distinta). Segunda de cuatro de la misma tanda (v1.4.44).
...
16. **`paths_deny` se chequea por archivo dentro de TODOS los walks recursivos (v1.4.83)**: antes, validar el directorio base con `resolve_and_validate(path)` bastaba para que `fs_search`/`fs_compress`/`fs_batch`/`fs_find`/`fs_list`/`fs_tree`/`fs_snapshot`/`fs_disk_usage`/`fs_find_duplicates` devolvieran contenido o nombres de archivos dentro de `paths_deny` (un `.env` con un secreto podía ser ex-filtrado vía `fs_search` aunque `fs_read` lo bloqueara). Ahora, cada uno de los 9 walks itera con `_walk_files_no_symlinks()` y aplica `SecurityValidator.is_denied_fast(archivo, "read")` por entrada — un patrón matcheado por archivo se omite y se contabiliza en un sufijo `[skipped N denied file(s) by paths_deny: patrón×M]` sin rutas (conteo por patrón, sin oráculo de existencia). La excepción de solo-lectura (`**/bin/**` con `.dll/.exe/.pdb`) sigue aplicando. **v1.4.85 (M2)**: además se podan DIRECTORIOS denied por el "core" del patrón vía `SecurityValidator.is_denied_fast_dir()` (un `**/node_modules/**` poda el dir `node_modules` en sí, que fnmatch nunca matcheaba por falta de `**` recursivo) — el generador `_walk_files_prune_denied()` los saca del `os.walk` antes de descender y los cuenta 1 (no N). Respetando `paths_deny_exceptions` (un `bin` exceptuado NO se poda). **Si agregás una tool nueva que recorra archivos**: aplicar el mismo helper + sufijo; el bypass es estructural, no se resuelve con validación del directorio base.
