...
- **Dos tests spawn más (mismo run)**: `test_sh_spawn_chained_all_segments_granted_passes` / `test_sh_spawn_chained_interpreter_segment_is_gated` otorgaban el grant `spawn:echo` con el token desnudo. En Windows `echo` es builtin de cmd (`shutil.which` → `None`, la key queda `spawn:echo`); en macOS/Linux `which("echo")` resuelve a `/bin/echo`, así que el gate pedía `spawn:/bin/echo` y el grant nunca matcheaba. Fix en los tests: los grants ahora usan `shutil.which("echo") or "echo"` — exactamente el mismo resolver que usa `_check_spawn_permission` (`layer2_shell.py`) por segmento, así que la key coincide en toda plataforma.
- **Bug de producción real descubierto al mismo tiempo (fix en `src/permissions.py`)**: el fallo de esos dos tests en Linux no era solo de keys — incluso sobre la key correcta (`spawn:/usr/bin/echo`) el grant podía ser inutilizable. `check_granted()` escaneaba el recurso `spawn:` contra `paths_deny`, y el default `**/bin/**` matcheaba el `/usr/bin/echo` de la ruta resuelta → `return False` ANTES de mirar el grant aprobado (además `_resolve()` prependía el CWD al key en POSIX: `Path("spawn:/usr/bin/echo").resolve()` → `<cwd>/spawn:/usr/bin/echo`). **Impacto en producción**: en Linux/macOS, un `sh_spawn` de cualquier binario bajo `/bin`/`/usr/bin` (y del venv de python, cuya ruta contiene `/bin/`) quedaba permanentemente imposible de aprobar, porque el deny ganaba sobre el grant en cada lookup. Fix: `_resolve()` devuelve los recursos `spawn:` intactos (son identidades de ejecutable, no paths — por eso el prefijo existe para separarlos de los grants `execute` de `sh_exec`) y `check_granted()` los despacha a `_check_identity_granted()`, que solo mira grants exactos por key y **omite paths_deny, el parent-walk recursivo y el auto-grant de data_dir** — paths_deny sigue ganando para archivos reales (test de regresión incluido). La lógica de consumo de grants SINGLE se extrajo a `_consume_single()` compartida por ambos caminos (una copia exacta, no dos divergentes).
- **Tests de regresión nuevos (5) en `tests/test_permissions.py`**: el deny default no se traga un grant `spawn:`; el key `spawn:` no se resuelve contra el CWD (grant+check coinciden byte-a-byte, `monkeypatch.chdir` en medio); el grant SINGLE `spawn:` se consume una única vez; el flujo real request+approve (`spawn:/usr/bin/echo`) queda consumible pese al deny; y `paths_deny` sigue ganando para rutas de archivo reales (.git) — el fix de spawn no afloja la frontera de archivos.
...
- **Parámetro `exclude`**: patrones fnmatch estilo `paths_deny` (`"**/node_modules/**"`, `"node_modules"`, `".venv"`, `"*.tmp"`) que excluyen directorios y archivos de la búsqueda. Los directorios matcheados se **podan** del recorrido (`dirnames[:]` antes de descender) — sin poda, `rglob` recorrería y hashearía miles de dependencias repetidas de `node_modules`/`.venv` (ruido real: no son basura del usuario) sin ahorrar tiempo. La búsqueda migró de `rglob("*")`/`iterdir()` a `os.walk` con el filtro de poda, reutilizando los helpers `_normalize_exclude_patterns`/`_excluded_by_patterns` de v1.4.79 (sin duplicar). Modo no-recursivo = walk de nivel 1 con poda igualmente aplicada. Default `None` = comportamiento idéntico al previo (test de paridad). Semántica de matching idéntica a `fs_disk_usage`: ruta relativa completa + nombre desnudo contra el "core" del patrón — `"**/node_modules/**"` matchea también un `node_modules` de primer nivel.
- **`min_size` (default `0`) / `max_size` (default `None`)**: filtros opt-in aplicados en fase 1, ANTES de agrupar — lo filtrado ni se agrupa ni se hashea (el ahorro es real, no cosmético). El default conserva el diseño documentado "sin caps" del repo (2026-07-31); los límites quedan disponibles cuando hacen falta (ej. acotar el hashing de imágenes de VM de varios GB duplicadas con `max_size`). Validación: `min_size < 0` y `max_size < min_size` devuelven `Error: ...` (estilo `sort_by`/`regex` existente).
- **Archivos vacíos (size 0) siempre fuera del reporte**: N `.gitkeep`/`__init__.py` vacíos formaban grupos de "duplicados" con 0 bytes recuperables — ruido puro. Excluidos de forma permanente (decisión 2026-08-16, con el usuario). Es el default consensuado de la industria, verificado contra fuentes primarias: jdupes `-n --noempty` ("this option is the default behavior", manpage Debian) y rmlint `--size` (default 1: "The default excludes empty files from the duplicate search", manpage Ubuntu); czkawka (1 KB) es la excepción minoritaria y queda cubierto pasando `min_size=1024` explícitamente.
- **Docstring completo del wrapper** (antes inexistente — gap vs `fs_disk_usage`/`project_git_status`): dos fases, ORIGINAL=st_ctime, `extensions`, `exclude`, `min_size`/`max_size`, vacíos siempre excluidos, flujo `fs_delete_batch`. Los 3 docstrings que decían "deliberately with no file-size cap" (sync/impl/wrapper) actualizados a "sin caps por defecto, acotables opt-in".
- **Tests nuevos**: 9 en `test_filesystem.py` — poda de subárbol excluido, patrón desnudo + archivos, paridad `exclude=None`, vacíos excluidos (3 vacíos + 1 par real → solo el par), `min_size` filtra, `min_size=0` incluye pequeños (paridad), `max_size` filtra, rango combinado, validación de valores inválidos. Suite completa: 529 passed, 1 skipped.
...
- **Gap cubierto**: `fs_write_batch`/`fs_delete_batch` existían desde v1.4.16/1.4.42 para operaciones batch, pero la edición `old_string`→`new_string` seguía siendo una llamada por archivo — N archivos implicaban N tickets y N popups. `fs_edit_batch` cierra el patrón: un solo ticket para la lista completa.
- **Diseño** (mismo razonamiento que `fs_delete_batch`/`fs_write_batch`): dedup de ediciones idénticas repetidas sobre la misma ruta (silencioso), rechazo up-front de la ruta repetida con `(old_string, new_string)` DISTINTO (instrucción ambigua — el batch completo se rechaza antes de tocar el disco, vía `_dedupe_edits()`); validación de TODAS las rutas con `validate_tool_paths_batch(paths, "write")` antes de consumir el grant; resumen `"N/M files edited"` con resultados y fallos por archivo.
- **Diferencias deliberadas vs. las otras dos batches**: (1) el loop vive en la función async, no en un helper sync en `asyncio.to_thread` — cada iteración debe `await _diff_or_timeout_note()` (async, envuelve difflib en su propio to_thread+timeout), y una función async no puede llamarse desde un worker thread sync; mismo patrón que `fs_read_multi_impl`. (2) Reusa `_diff_or_timeout_note()` por archivo, NO `difflib` directo — reabrir ese camino reintroduciría el hang que costó a `fs_edit` 4+ minutos dos veces (2026-08-08, ver `_DIFF_TIMEOUT_SECONDS`).
- **Errores por archivo, no globales**: archivo inexistente reporta "not a file or does not exist" (razonamiento M-F1 de `fs_edit_impl`, no el engañoso "old_string not found"); cada fallo individual se loguea como `fs_edit_batch FAIL path=...` (misma convención de `fs_delete_batch`/`fs_write_batch`, 2026-08-07) para diagnóstico post-mortem desde `server.log`.
- **Sin re-pasar la operación**: el wrapper consume el grant de batch `"write"`; `fs_edit_batch_impl` llama a `resolve_and_validate()` con el default `"read"` (misma convención que las otras impls batch).
- **Auditoría cubierta**: `fs_write_batch`/`fs_edit_batch` agregadas a `_SEMANTIC_FAILURE_TOOLS` (`server.py`) — sin esto, un batch bloqueado por tickets se habría auditado como `OK` en vez de `BLOCKED`. AGENTS.md regla 7 actualizada con ambas. Tests: 2 nuevos en `test_audited_fastmcp.py`.
- **Tests nuevos**: 7 en `test_filesystem.py` — básico 2 archivos, inexistente, old_string no encontrado (archivo intacto), fallos individuales logueados, dedup idéntico sin error, conflictos rechazados sin tocar disco, y timeout de diff que no bloquea el batch (monkeypatch de `_DIFF_TIMEOUT_SECONDS`/`_unified_diff_sync`). Suite completa: 507 passed, 1 skipped.

### Fixed — `sh_spawn` generaba ticket nuevo por cada script Python distinto aunque el intérprete ya tuviera grant de sesión

- **Causa raíz (dos capas)**: (1) `_check_spawn_permission()` usaba el comando completo como resource key (`spawn:python -c "...script completo..."`) — cada script diferente generaba un resource distinto y por tanto un ticket nuevo, aunque el ejecutable fuera el mismo. (2) `shutil.which("python")` resolvía al Python del sistema (`C:\Python314\python.exe`) mientras el servidor corre en el venv (`C:\Users\usuario\Repos\.personal-mcp\.venv\Scripts\python.exe`) — el grant aprobado para uno nunca coincidía con el check del otro. El mismo problema de resolución afectaba a `validate_shell_execution()` en `security.py` para comandos `sh_exec`.
- **Fix `layer2_shell.py`**: `_check_spawn_permission()` normaliza el resource a solo el ejecutable resuelto (`spawn:<exe_path>`) en lugar del comando completo. Para `python`, usa `sys.executable` (el intérprete del venv que corre el servidor) en lugar de `shutil.which()`.
- **Fix `security.py`**: `validate_shell_execution()` aplica la misma lógica — `sys.executable` para `python`, `shutil.which()` para el resto. Agrega `import sys` (faltaba).
- **Efecto**: un único grant de sesión para el intérprete Python cubre cualquier invocación de `sh_spawn` con ese intérprete, sin importar el script. Ojo: los grants de spawn llevan el prefijo `spawn:` y son SEPARADOS por diseño de los de `sh_exec` (AGENTS.md: "sh_spawn exige su propio ticket de execute") — un grant aprobado vía `sh_exec` para python.exe NO cubre `spawn:python.exe` y viceversa. El `sys.executable` alinea el fix con lo que realmente se ejecuta (venv), que era la segunda capa del bug.
- **Hallazgo adicional de la revisión de seguridad (2026-08-16, corregido en el mismo 1.4.78)**: `_check_spawn_permission()` solo miraba el PRIMER token del comando, mientras `validate_shell_execution()` (sh_exec) itera segmento por segmento. `sh_spawn("echo hi; python -c '...'")` pedía ticket para `spawn:echo` — y una vez aprobado echo, el intérprete python del segundo segmento corría SIN ningún ticket de execute: bypass silencioso del gate de intérpretes que sh_exec no tiene. Fix: `_check_spawn_permission()` ahora itera TODOS los segmentos de `split_command_segments()` (mismo patrón que `validate_shell_execution`) — cada segmento exige su propio grant de spawn. Tests: 2 nuevos en `test_shell.py` (segmento intérprete aún gatillado cuando el primero ya tiene grant; pase solo cuando TODOS los segmentos tienen grant).
- **Tests**: el fix original quedó cubierto por los 2 tests nuevos del hallazgo de arriba (el resource key `spawn:<sys.executable>` se verifica explícitamente). Suite completa: 509 passed, 1 skipped.

### Changed — tickets batch (`fs_delete_batch`/`fs_write_batch`/`fs_edit_batch`): hint de sesión en el mensaje + preview acotado de archivos

- **Punto 1 — hint `level='session'` en el mensaje del ticket batch**: el JSON `permission_required` de `validate_tool_paths_batch()` solo ofrecía `level='single'`, mientras `request_permission()` (tickets de un recurso) ofrecía session desde v1.4.28 — el agente no tenía forma de saber que un batch write/edit podía aprobarse "de una vez para toda la sesión". Ahora el mensaje sugiere `level='single'` para un solo uso **o** `level='session'` para autorizar todo el lote por la sesión (patrón "ask once, session-scoped"); para `delete` el hint aclara explícitamente que es siempre single-use y que `level='session'` no está permitido — `approve()` lo fuerza de todos modos (`permissions.py:357`), el hint solo evita llamar con un nivel que el servidor ignora silenciosamente.
- **Punto 2 — resumen acotado de archivos en el mensaje**: el mensaje del ticket batch ahora muestra los primeros `_BATCH_PREVIEW_MAX=5` paths + `"... y N archivo(s) más"` (la lista completa sigue viajando intacta en el campo `resources`, que es la fuente real para `fs_approve`/`security_pending`). Antes el mensaje solo decía el conteo sin ninguna referencia a qué archivos; listarlos todos habría inundado el contexto del agente con listas enormes.
- **Punto 3 — popup batch (`oslayer/confirm.py`)**: el cuerpo del mensaje batch estaba duplicado en las tres plataformas y ponía el código de confirmación al FINAL — MessageBoxW/zenity no hacen scroll, así que una lista larga empujaba el código fuera de la ventana y el usuario no podía autorizar nada (incidente real reportado: "si se pasa una lista grande se va a perder el código"). Nuevo helper compartido `_build_batch_message()` (elimina la duplicación de los tres `_show_*`): el **código de confirmación va SIEMPRE primero** y la preview de archivos se acota a `MAX_PREVIEW_FILES=10` sin importar `len(files)`. La lista completa queda disponible vía `security_pending`/`resources`.
- **Tests nuevos**: 3 en `test_security.py` (batch write sugiere session; batch delete nunca sugiere session y menciona single-use; preview acotado con 8 paths — primeros 5 en el mensaje, `resources` intacto con los 8) + 2 en `test_permissions.py` para `_build_batch_message` (código siempre primero + preview acotado; sin truncar dentro del límite). Suite completa: 516 passed, 1 skipped.

## [1.4.77] — 2026-08-15

### Added — campo `instructions` del protocolo MCP, para no depender de que el agente lea AGENTS.md por su cuenta
- **Contexto**: la convencion de cerrar cada sesion con `journal_add()` se agrego a `AGENTS.md` a mitad de una conversacion (commit `750371f`). El agente en esa misma conversacion nunca la vio -- solo consultaba `AGENTS.md` con `grep` puntual sobre otros temas, nunca con una lectura completa despues de que se agrego. Verificado en vivo: `journal_stats` mostro 5 entradas en toda la sesion, todas de otra sesion/herramienta (opencode), ninguna del agente que participó en esta conversacion.
- **Fix**: `build_server_instructions(journal_enabled)` (nueva, `server.py`) construye un texto corto (no vuelca `AGENTS.md` completo, 150+ lineas, pensado para lectura bajo demanda) que se pasa como `instructions=` a `AuditedFastMCP.__init__()` -- campo real del protocolo MCP, confirmado en la libreria instalada (`inspect.signature(FastMCP.__init__)`), no supuesto. Condicional a `journal.enabled`: si esta deshabilitado, Layer 4 completo (incluido `journal_add`) no se registra -- instruir sobre una tool inexistente seria peor que no decir nada.
- **Límite honesto, verificado un nivel más — hasta dónde llega y dónde deja de ser verificable**: rastreado en la librería instalada, no supuesto — `FastMCP.__init__` pasa `instructions` directo al `MCPServer` de bajo nivel (`self._mcp_server = MCPServer(..., instructions=instructions, ...)`), que a su vez lo guarda como `self.instructions` y lo usa para construir `InitializationOptions(..., instructions=self.instructions, ...)` — el objeto que arma la respuesta real de `initialize` enviada por el protocolo. La cadena servidor→wire está confirmada de punta a punta, no es un atributo decorativo. Lo que sigue sin poder verificarse desde acá es exclusivamente el lado cliente: si Claude Desktop lee ese campo de la respuesta y me lo muestra, y de qué forma — eso ya no depende de nada inspeccionable desde este repo.
- Funcion pura, separada de `create_app()` (que tiene efectos secundarios: carga config real, configura logging) para poder testearla sin esos efectos.
- **Tests nuevos**: 2 en `test_audited_fastmcp.py` (instructions presente con journal habilitado, `None` sin journal). Verificado ademas con `create_app()` real: `app.instructions` queda seteado correctamente con el `journal.enabled: true` de este equipo.
- Verificado: 505/505 (2 skipped), suite completo.

## [1.4.76] — 2026-08-15

### Fixed — grants `single` de escritura se gastaban en intentos que nunca tocaban el archivo

- **Contexto**: reportado por el usuario desde su propia experiencia ("editar un archivo con ticket a veces hace que la edicion no se efectue, solicitar otro ticket a veces hace que la ventana que lo suministra no se abra"). Confirmado trazando el orden real de ejecución de `fs_edit`/`fs_edit_advanced`/`fs_write`, no por documentación/comentarios propios del código.
- **Causa raíz**: en los tres wrappers, `validate_tool_path(path, "write")` consume el grant `single` (si es lo que autoriza la llamada) *antes* de que el impl correspondiente pueda comprobar si la operación es siquiera posible: `fs_edit_impl` valida recién después si `old_string` está en el contenido actual; `fs_edit_advanced_impl` valida cada `oldText` de la lista, y el propio wrapper valida `edits` no vacío; `fs_write_impl` valida `max_size_mb` contra el tamaño real del contenido. Un desajuste en cualquiera de esos tres — contenido desactualizado, `edits` vacío, contenido más grande que el límite — deja el archivo intacto pero el ticket ya gastado, obligando a pedir uno nuevo (y por lo tanto un popup nuevo) para reintentar algo que nunca tocó el disco.
- **Fix**: `PermissionManager.has_single_grant(resource, operation)` (peek sin consumir; devuelve la key exacta que `check_granted` consumiría — la operación o el comodín `"*"` — o `None` si el acceso vendría de un grant de sesión/permanente/`data_dir`, caso en el que no hay nada que reembolsar) y `refund_single(resource, operation_key)` (restaura una unidad), expuestos en `SecurityValidator` como pass-through delgado. Los tres wrappers (`fs_edit`, `fs_edit_advanced`, `fs_write`) hacen el peek antes de `validate_tool_path` y reembolsan si el resultado final empieza con `"Error:"` — cubierto: `old_string`/`oldText` no encontrado, archivo inexistente, `edits` vacío, `content too large`. `fs_edit_advanced` con `dry_run=True` reembolsa incondicionalmente (nunca toca el disco, gane o pierda el preview de matching), no solo en caso de error.
- **Por qué `has_single_grant` devuelve la key y no un bool**: un grant `single` puede estar guardado bajo la operación exacta (`"write"`) o bajo el comodín `"*"` (que `check_granted` también acepta para operaciones que no son `delete`/`execute`). Reembolsar a ciegas contra `"write"` cuando lo consumido fue `"*"` crearía un grant nuevo en la key equivocada en vez de restaurar el que se gastó.
- **Por qué el peek es obligatorio antes de reembolsar (no reembolsar sin condición)**: si el acceso vino de un grant de sesión, `check_granted` no consume nada — reembolsar igual fabricaría un grant `single` que nunca existió, ensanchando el acceso más allá de lo aprobado. `has_single_grant` distingue ambos casos antes de que `validate_tool_path` mute el estado.
- **Corregido en el mismo cambio**: `fs_edit_advanced_impl` no validaba `is_file` antes de leer, a diferencia de `fs_edit_impl` (que sí lo hace desde M-F1, 2026-08-11) — sobre un archivo inexistente, intentaba matchear `oldText` contra el string de error que devuelve `fs_read_impl` en vez de reportar el problema real, dando un `"'oldText' not found"` engañoso. Mismo fix aplicado: `is_file` check antes de leer.
- **Tests nuevos**: 3 unitarios directos sobre `PermissionManager` en `test_permissions.py` (roundtrip peek+consume+refund, no fabricar grant sobre uno de sesión, key `"*"`); 6 de integración en `test_filesystem.py` — 5 vía `register_filesystem_tools`+`AuditedFastMCP` (necesario porque el fix de reembolso vive en el wrapper, no en `_impl` — la convención habitual de importar `_impl` directo no lo cubre): reembolso en `fs_edit` por contenido desactualizado, no-fabricación cuando la autorización es por sesión, reembolso en `fs_edit_advanced` por `edits` vacío y por `dry_run`, reembolso en `fs_write` por `content too large`; más 1 directo sobre `fs_edit_advanced_impl` (archivo inexistente, análogo a `test_edit_nonexistent_file_returns_error`).
- **Verificado**: suite completa `pytest -q` → 498 passed, 1 skipped (subió de 495 tras este cambio completo; el único warning es una `ResourceWarning` preexistente y no relacionada en `test_shell_resolver.py`).

## [1.4.75] — 2026-08-15

### Fixed — approve() no re-mostraba el popup de confirmacion salvo en tickets restaurados tras un reinicio
- **Contexto**: reportado por el usuario desde su propia experiencia ("a veces no se abre por segunda vez para la misma edicion, si se suministra un codigo equivocado y luego se le solicita al agente uno nuevo"). Confirmado trazando la logica real de `approve()`, no por documentacion/comentarios del propio codigo.
- **Causa raiz**: `approve()` solo llamaba `show_confirmation_code()`/`show_confirmation_code_batch()` de nuevo cuando `ticket.restored` era `True` (ticket reconstruido desde `tickets.jsonl` tras un reinicio, v1.4.41). Para un ticket vivo del proceso actual -- el caso normal, la inmensa mayoria -- un codigo incorrecto devolvia el error sin volver a mostrar el codigo correcto en ningun lado. La unica forma de verlo de nuevo era que el agente reintentara la llamada ORIGINAL (`fs_edit`/`fs_request_allow`, que si re-muestra via el dedup existente en `request()`/`request_batch()`) en vez de `fs_approve()` -- algo que ningun agente sabria sin leer el codigo fuente.
- **Sin cobertura previa**: cero tests mockeaban/verificaban que `show_confirmation_code` fuera efectivamente llamada -- explica por que nadie detecto la asimetria entre `request()` (siempre re-muestra) y `approve()` (solo si `restored`).
- **Fix**: se remueve la condicion `if ticket.restored:` -- el re-mostrado ahora es incondicional en cualquier intento con codigo invalido, acotado por el rate-limiting existente (`_MAX_APPROVE_ATTEMPTS=10`, sin cambios). Mismo comportamiento que ya tenian `request()`/`request_batch()`, ahora unificado en los tres.
- **Tests nuevos**: 3 en `test_permissions.py`, con `unittest.mock.patch` sobre `show_confirmation_code`/`_batch` -- ticket vivo (no restaurado) con codigo incorrecto re-muestra el popup individual y el de batch; el intento que alcanza el limite de reintentos NO re-muestra (ticket ya queda `denied`, no hay codigo que mostrar).
- Verificado: 56/56 en `test_permissions.py` + `test_ticket_persistence.py` (el camino `restored` sigue sin cambios de comportamiento), 492/492 (1 skipped) en el suite completo.
