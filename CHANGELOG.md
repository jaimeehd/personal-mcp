...
- **Dos tests spawn más (mismo run)**: `test_sh_spawn_chained_all_segments_granted_passes` / `test_sh_spawn_chained_interpreter_segment_is_gated` otorgaban el grant `spawn:echo` con el token desnudo. En Windows `echo` es builtin de cmd (`shutil.which` → `None`, la key queda `spawn:echo`); en macOS/Linux `which("echo")` resuelve a `/bin/echo`, así que el gate pedía `spawn:/bin/echo` y el grant nunca matcheaba. Fix en los tests: los grants ahora usan `shutil.which("echo") or "echo"` — exactamente el mismo resolvers que usa `_check_spawn_permission` (`layer2_shell.py`) por segmento, así que la key coincide en toda plataforma. Sin cambios en producción: el comportamiento del gate por segmento era correcto.
