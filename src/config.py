...
def _default_paths_deny() -> list[str]:
    ...
    if sys.platform == "win32":
        # A-6 (auditoría 2026-08-11): the previous exact-path entry
        # (str(home / "AppData")) had no wildcard, so fnmatch treated it as an
        # exact match -- it blocked only the literal directory, never anything
        # inside it. With paths_allow potentially covering the whole drive, every
        # file under AppData was readable. Must be a recursive wildcard like every
        # other deny entry (matches the real config's `**\AppData\**`).
        base.append("**/AppData/**")
    else:
...
class CommandPolicy(BaseModel):
    ...
    def is_command_allowed(self, command: str) -> tuple[bool, str]:
        ...
        from src.shell_resolver import has_unsafe_substitution, split_command_segments
        if has_unsafe_substitution(command):
            return False, (
                "Command blocked: command substitution ($() or backticks) inside "
    ...
    def is_script_readonly(self, script: str) -> tuple[bool, str]:
        ...
        line must be made up entirely of read-only segments.
        ...
        Each line is split into shell segments (on | ; & < > ` $( \\n) and every
        segment must start with a read-only prefix -- not just the line as a whole.
        This closes the inline-operator bypass where `echo x; Remove-Item ...` passed
        because only the first prefix was checked (C-1, auditoría 2026-08-11).
        Command substitution inside double quotes ($()/backticks) is rejected
        outright -- split_command_segments() cannot see inside quotes, but the shell
        still expands it (C-2).
        """
        ...
        for i, raw_line in enumerate(script.splitlines(), 1):
            ...
            if not line or line.startswith("#"):
                continue
            if has_unsafe_substitution(line):
                return False, (
                    f"Line {i} uses command substitution ($() or backticks), which is "
                    f"not allowed in read-only scripts: '{line[:60]}'"
                )
            for segment in split_command_segments(line) or [line]:
                seg_lower = segment.strip().lower()
                if not seg_lower:
                    continue
...
class AppConfig(BaseModel):
    ...
    def load(cls, path: Path | None = None) -> "AppConfig":
        ...
        if path.exists():
            # B-4 (auditoría 2026-08-11): a single invalid UTF-8 byte in the
            # user's config.json raised UnicodeDecodeError (crash). Replace it
            # and let the JSON parse report a real error if the file is broken.
