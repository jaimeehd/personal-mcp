...
def _show_linux(resource: str, operation: str, code: str, batch: bool, files: list[str] | None = None) -> None:
    ...
    def _write_fallback() -> None:
        """Last resort: write code to a 0600 temp file and log only the path.

        M-H3 (auditoría 2026-08-11): the previous fallback printed the raw
        confirmation code to stderr, which can end up in a shared server.log that
        the agent can read via mcp_log — reopening the auto-approval gap this
        module exists to close. The code must never leave this process as plain
        text on a channel the agent can observe; only the (owner-only) file path
        is reported.
        """
        import tempfile
        try:
            fd, path = tempfile.mkstemp(prefix="personal-mcp-confirm-", suffix=".txt")
            with os.fdopen(fd, "w", encoding="utf-8") as f:
                os.chmod(path, 0o600)
                f.write(f"{title}\n\n{message}\n")
            print(f"[CONFIRMATION CODE] written to {path} (0600, owner-only)", file=sys.stderr, flush=True)
        except Exception:
            print("[CONFIRMATION CODE] could not write fallback file", file=sys.stderr, flush=True)
...
def _show_macos(resource: str, operation: str, code: str, batch: bool, files: list[str] | None = None) -> None:
    ...
    # M-H4 (auditoría 2026-08-11): `resource`/`operation`/`code` were interpolated
    # raw into an AppleScript double-quoted string, so a `"` or `\` in the resource
    # path could break out and inject arbitrary AppleScript. Escape for AppleScript
    # string literals and flatten newlines (a newline inside the literal is invalid).
    def _esc(s: str) -> str:
        return s.replace("\\", "\\\\").replace('"', '\\"').replace("\n", " ")

    script = f'display dialog "{_esc(message)}" with title "{_esc(title)}" buttons {{"OK"}} default button "OK"'
