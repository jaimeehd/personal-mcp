...
class TestEdgeCases:
    ...
    async def test_empty_paths_allow_blocks_all_except_data_dir(self, temp_home):
        config = AppConfig(
            security=SecurityConfig(
                paths_allow=[],
                # Empty deny: this test is about paths_allow being empty, not the
                # deny default. The default `**/AppData/**` would also match the
                # tmp_path-based data_dir here (pytest tmp lives under AppData).
                paths_deny=[],
            ),
            data_dir=str(temp_home / ".personal-mcp" / "data"),
        )
