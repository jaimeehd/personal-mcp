...
from src.layers.layer4_personal import (
    Journal,
    journal_add_impl,
    note_quick_impl,
    project_git_status_impl,
)
...
# --- M-P1 (auditoría 2026-08-11): secret scan still fires (scan runs pre-write) ---

def test_journal_add_warns_on_secret(journal, test_config):
    result = journal_add_impl("AKIAABCDEFGHIJKLMNOP", journal, test_config)
    assert "saved" in result
    assert "Security Scan" in result


def test_note_quick_warns_on_secret(test_config):
    result = note_quick_impl("AKIAABCDEFGHIJKLMNOP", test_config)
    assert "saved" in result
    assert "Security Scan" in result


# --- M-P4 (auditoría 2026-08-11): git returncode surfaced ---

def test_git_project_info_corrupt_git_reports_error(temp_home):
    from src.layers.layer4_personal import _git_project_info

    d = temp_home / "Repos" / "corrupt_repo"
    d.mkdir()
    (d / ".git").mkdir()
    info = _git_project_info(d)
    assert info["branch"] == "(error)"
