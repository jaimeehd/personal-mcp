...
def test_deny_exception_allows_matching_dll_read_nested(exception_security, temp_home):
    # bin\ bajo una subcarpeta intermedia (ej. src\MyProj\bin\...) -- matchea el patron con "**\bin\**".
...
def test_multiple_allowed_paths(temp_home):
    config = AppConfig(
        security=SecurityConfig(
            ...
            # Explicit deny: pytest's tmp_path lives under AppData\\Local\\Temp, so
            # the (correct) default `**/AppData/**` would match it here. This test
            # is about paths_allow multiplicity, not the deny default (A-6).
            paths_deny=["**/node_modules/**", "**/.git/**"],
        ),
        data_dir=str(temp_home / ".personal-mcp" / "data"),
    )
...
def test_risky_script_ticket_uses_clean_resource_key(strict_security, temp_home):
    strict_security.config.security.commands.approval_required_prefix = ["python"]
    script_path = temp_home / "Repos" / "risky.py"
    script_path.write_text("import socket\n")

    result = strict_security.validate_shell_execution(f'python "{script_path}"')
...
def test_risky_script_approval_actually_grants_access(strict_security, temp_home):
    strict_security.config.security.commands.approval_required_prefix = ["python"]
    script_path = temp_home / "Repos" / "risky2.py"
    script_path.write_text("import socket\n")
    ...
    ticket = strict_security.perm_manager._tickets[ticket_id]
    ok, _msg = strict_security.perm_manager.approve(
        ticket_id, level=GrantLevel.SESSION, confirm_code=ticket.confirm_code
    )
    assert ok
...
# --- A-3 (auditoría 2026-08-11): relative/tilde path detection ---

def test_extract_traversal_paths_relative():
    paths = SecurityValidator.extract_traversal_paths("cat ../secret.txt")
    assert "../secret.txt" in paths


def test_extract_traversal_paths_home_relative():
    paths = SecurityValidator.extract_traversal_paths("ls ~/.ssh")
    assert "~/.ssh" in paths


def test_extract_traversal_paths_git_notation_not_flagged():
    # git range (`..`) and rev (`~1`) notation must not be treated as paths.
    assert SecurityValidator.extract_traversal_paths("git log ..HEAD") == []
    assert SecurityValidator.extract_traversal_paths("git log HEAD~1") == []


# --- A-5 (auditoría 2026-08-11): read-only deny exception must not apply to delete ---

def test_deny_exception_does_not_apply_to_delete(exception_security, temp_home):
    dll_path = temp_home / "Repos" / "MyProj" / "bin" / "Release" / "MyProj.dll"
    dll_path.parent.mkdir(parents=True, exist_ok=True)
    dll_path.write_bytes(b"fake dll content")
    # read: the read-only deny exception applies
    assert exception_security.resolve_and_validate(str(dll_path)) == dll_path.resolve()
    # delete: deny must win — the read-only exception never applies to destructive ops
    with pytest.raises(PathNotAllowedError, match="Path denied by pattern"):
        exception_security.resolve_and_validate(str(dll_path), "read", deny_operation="delete")


def test_batch_delete_blocks_deny_excepted_path(exception_security, temp_home):
    dll_path = temp_home / "Repos" / "MyProj" / "bin" / "Release" / "MyProj.dll"
    dll_path.parent.mkdir(parents=True, exist_ok=True)
    dll_path.write_bytes(b"fake dll content")
...
def test_validate_command_enforces_rate_limit(strict_security):
    strict_security.config.security.rate_limit_commands_per_minute = 2
